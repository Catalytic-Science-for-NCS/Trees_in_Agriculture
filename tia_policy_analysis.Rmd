...
wide_wb<-subset(policy_wide,select=c(general_cols[1:5],wheatB_cols,wheatNB_cols),value=T)
wide_cb<-subset(policy_wide,select=c(general_cols[1:5],cornB_cols,cornNB_cols),value=T)
wide_sb<-subset(policy_wide,select=c(general_cols[1:5],soyB_cols,soyNB_cols),value=T)

#subset trees in fields data
wide_wf<-subset(policy_wide,select=c(general_cols[1:5],wheatF_cols,wheatNF_cols),value=T)
wide_cf<-subset(policy_wide,select=c(general_cols[1:5],cornF_cols,cornNF_cols),value=T)
wide_sf<-subset(policy_wide,select=c(general_cols[1:5],soyF_cols,soyNF_cols),value=T)

#subset policy  data
wide_p<-subset(policy_wide,select=c(general_cols[1:5],policy_cols,policyN_cols),value=T)
```
...
policy_long<-
  ...
  full_join(long_wf)%>%
  full_join(long_sf)%>%
  mutate_at(vars(reasonImportance),factor,levels=c("Top","Second","Third","Fourth","Fifth","Sixth","Seventh","Eighth","Last","Not Meaningful"))
...
# Visualize Data
_For now this only allows choices for crop, with other defaults applied_
- Defaults
...
```{r user inputs}
ui<-fluidPage(
  titlePanel("Custom Histogram"),
  sidebarLayout(
    sidebarPanel(
      helpText("Create histogram with information from expert elicitation.A \"unique response\" is a unique combination of expert respondent and a tree species they recommended."),
      selectInput("crop", label = "Choose a variable to display",choices = c("corn","wheat","soy","All"), selected = "All")
      ),
mainPanel(plotOutput("custom_plot1"))
)
)
...
#selectInput("what2plot",label="What to Plot", 
#            choices=c("Reasons for Trees in Boundaries","Reasons for Trees in Fields","Reasons for Trees in Fields or Boundaries","Policy Recommendations"), selected = "Policy Recommendations")
#selectInput("continent", label = "Continent:",
#              choices = c("Africa", "Asia (includes Middle East)", "Europe", "North/Central America (includes Caribbean)","South America","All"), selected = "All")

#selectInput("biome",label="Biome:",
 #           choices= c("Boreal Forests & Taiga","Mediterranean Forests; Woodland; & Scrub", "Temperate Broadleaf & Mixed Forests", "Tropical & Subtropical Dry Broadleaf Forests", "Tropical & Subtropical Moist Broadleaf Forests","Temperate Grasslands; Savannas & Shrublands","Tropical & Subtropical Grasslands; Savannas; and Shrublands", "All"), selected = "All")


```
2. Display custom plot based on user inputs
```{r customize plot}
...
server<-function(input, output){
  output$custom_plot1<-renderPlot({
    crop_histogram(crop=input$crop,data=policy_long,what2plot=what2plot,continent=continent,biome=biome)
    })
}

shinyApp(ui=ui,server=server)

#if(what2plot!="Policy Recommendations"){}
