...
bios_sim <- bios_sim[bios_sim$BIOME_NAME!= "Boreal Forests/Taiga" & bios_sim$BIOME_NAME!="Mangroves" & bios_sim$BIOME_NAME!="NA" & bios_sim$BIOME_NAME!="Flooded Grasslands & Savannas",]
...
pairs <- inner_join(bios_sim, rootshoot, by=c("BIOME_NAME")) %>%
  vect()
names(pairs) <- c("BIOME_NUM","BIOME_NAME","drop","Equivalent_Mokany_system","Median_rootShoot_ratio","SE")
