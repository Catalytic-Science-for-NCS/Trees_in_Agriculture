...
agarea[agarea==0] <- NA
agarea <- agarea[!duplicated(agarea),]
...
agarea_byBiome <- agarea %>%
  ...
  summarize(CropArea_ha = sum(CropAreaByBiomeCountry_tic_ha, na.rm=T),
            GrazeArea_ha = sum(GrazeAreaByBiomeCountry_tip_ha,na.rm = T))


crop_extent = sum(agarea$CropAreaByBiomeCountry_tic_ha, na.rm = T)
graze_extent = sum(agarea$GrazeAreaByBiomeCountry_tip_ha, na.rm = T)
...
flx <- get_metric(pattern="FluxDensityByBiomeCountry_mean", sum_or_mean_column="mean")
any(isFALSE(flx[,c("ISO3", "BIOME_NAME","BIOME_NUM")]==agarea[,c("ISO3", "BIOME_NAME","BIOME_NUM")]))

if((nrow(flx)==nrow(agarea))){
flx_df <- cbind(flx, agarea[,c("CropAreaByBiomeCountry_tic_ha", "GrazeAreaByBiomeCountry_tip_ha")])
} else {
  break
}
...
country_flx <- flx_df %>%
  group_by(ISO3) %>%
  summarize(crop_C_perCountry = weighted.mean(tic, CropAreaByBiomeCountry_tic_ha, na.rm=T),
            graze_C_perCountry = weighted.mean(tip, GrazeAreaByBiomeCountry_tip_ha, na.rm=T))
            #CropArea_tic_ha= sum(CropAreaByBiomeCountry_tic_ha, na.rm = T),
            #GrazeArea_tip_ha = sum(GrazeAreaByBiomeCountry_tip_ha, na.rm=T))

biome_flx <- flx_df %>%
  group_by(BIOME_NAME) %>%
  summarize(crop_C_perBiome = weighted.mean(tic, CropAreaByBiomeCountry_tic_ha, na.rm=T),
            graze_C_perBiome = weighted.mean(tip, GrazeAreaByBiomeCountry_tip_ha, na.rm=T))
            #CropArea_tic_ha= sum(CropAreaByBiomeCountry_tic_ha, na.rm = T),
            #GrazeArea_tip_ha = sum(GrazeAreaByBiomeCountry_tip_ha, na.rm=T))


weighted.mean(flx_df$tic, flx_df$CropAreaByBiomeCountry_tic_ha, na.rm=T)
...
sum(country_cacc$crop_C_perCountry, na.rm=T)/(crop_extent/1000000)/30/1000000 
sum(country_cacc$graze_C_perCountry, na.rm=T)/(graze_extent/1000000)/30/1000000
sum(biome_cacc$crop_C_perBiome, na.rm=T)/(crop_extent/1000000)/30/1000000
sum(biome_cacc$graze_C_perBiome, na.rm=T)/(graze_extent/1000000)/30/1000000
...
colnames(cover) <- c("ISO3", "NAME", str_extract(gsub(".csv", "", basename(fls[[1]])), "tic|tip"), 
                      str_extract(gsub(".csv", "", basename(fls[[2]])), "tic|tip"))
...
ctry_df$crop_flux <- ctry_df$crop_C_perCountry.x/(ctry_df$CropArea_ha/1000000)/30/1000000
ctry_df$graze_flux <- ctry_df$graze_C_perCountry.x/(ctry_df$GrazeArea_ha/1000000)/30/1000000
...
write.csv(ctry_df, "data/02_17_2023/country_results_03082023.csv")
...
colnames(biome_df) <- c("BIOME_NUM", "Crop_ForestCover_mean_percent", "Graze_ForestCover_mean_percent",
...
col_order <- c("BIOME_NUM","BIOME_NAME","Crop_TotalC_sum_MgC","Graze_TotalC_sum_MgC",
               "Crop_FluxDensity_mean_MgC/ha_CORRECT","Graze_FluxDensity_mean_MgC/ha_CORRECT",
               "Crop_FluxDensity_mean_MgC/ha_WRONG","Graze_FluxDensity_mean_MgC/ha_WRONG",
               "Crop_Area_sum_ha","Graze_Area_sum_ha", "Crop_ForestCover_mean_percent", "Graze_ForestCover_mean_percent",
               "Crop_Delta_mean_percent", "Graze_Delta_mean_percent")
biome_df <- biome_df[, col_order]

write.csv(biome_df, "data/02_17_2023/biome_results_03082023.csv")
